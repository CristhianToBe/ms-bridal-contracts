...
from .services import DocumentService, NotionService
...
def run_word_view(request):
    ...
    required_fields = {
        "word_base": "Falta 'word_base' (Plantilla Word). Revisa el name del <select>.",
        "mapping_json": "Falta 'mapping_json' (archivo de mapping).",
        "output_name": "Debes indicar el nombre del archivo Word de salida.",
        "json_text": "No se recibió contenido JSON para Word.",
    }
    ...
    out_name = request.POST.get("output_name", "").strip()
    ...
    try:
        ...
        return HttpResponse(f"JSON inválido: {error}", status=400)

    try:
        notion_resp = NotionService.create_contract_row(payload)
        print("✅ Notion page:", notion_resp.get("id"))
    except NotionError as error:
        return HttpResponse(f"Falló envío a Notion: {error}", status=500)

    try:
        zip_buffer, zip_name = DocumentService.generate_word_package(
            base_docx=base_docx,
...
def run_excel_view(request):
    ...
    required_fields = {
        "excel_base": "Falta 'excel_base' (Plantilla Excel).",
        "excel_mapping": "Falta 'excel_mapping' (archivo de mapping).",
        "excel_out_name": "Debes indicar el nombre del archivo Excel de salida.",
        "excel_json_text": "No se recibió contenido JSON para Excel.",
    }
    for field, message in required_fields.items():
        if not (request.POST.get(field) or "").strip():
            return HttpResponse(message, status=400)
    ...
    try:
        zip_buffer, zip_name = DocumentService.generate_excel_package(
